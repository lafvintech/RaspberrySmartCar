import time
from Motor import *
import RPi.GPIO as GPIO
from servo import *
from PCA9685 import PCA9685
import random

class Ultrasonic:
    def __init__(self):
        GPIO.setwarnings(False)
        self.trigger_pin = 27
        self.echo_pin = 22
        self.MAX_DISTANCE = 300  # define the maximum measuring distance, unit: cm
        self.timeOut = self.MAX_DISTANCE * 60  # calculate timeout according to the maximum measuring distance
        GPIO.setmode(GPIO.BCM)
        GPIO.setup(self.trigger_pin, GPIO.OUT)
        GPIO.setup(self.echo_pin, GPIO.IN)

    def pulseIn(self, pin, level, timeOut):  # obtain pulse time of a pin under timeOut
        t0 = time.time()
        while (GPIO.input(pin) != level):
            if ((time.time() - t0) > timeOut * 0.000001):
                return 0;
        t0 = time.time()
        while (GPIO.input(pin) == level):
            if ((time.time() - t0) > timeOut * 0.000001):
                return 0;
        pulseTime = (time.time() - t0) * 1000000
        return pulseTime

    def get_distance(self):  # get the measurement results of ultrasonic module,with unit: cm
        distance_cm = [0, 0, 0, 0, 0]
        for i in range(5):
            GPIO.output(self.trigger_pin, GPIO.HIGH)  # make trigger_pin output 10us HIGH level
            time.sleep(0.00001)  # 10us
            GPIO.output(self.trigger_pin, GPIO.LOW)  # make trigger_pin output LOW level
            pingTime = self.pulseIn(self.echo_pin, GPIO.HIGH, self.timeOut)  # read plus time of echo_pin
            distance_cm[i] = pingTime * 340.0 / 2.0 / 10000.0  # calculate distance with sound speed 340m/s
        distance_cm = sorted(distance_cm)
        final_distance = distance_cm[2]  # 取排序后的中间值

        # 如果测量值等于0，转换为255
        if final_distance == 0:
            final_distance = 255

        return int(final_distance)  # 返回处理后的整数距离值
        #return int(distance_cm[2])
    """
    def run_motor(self, L, M, R):
        if (L < 30 and M < 30 and R < 30) or M < 30:
            self.PWM.setMotorModel(-1000, -1000, -1000, -1000)
            time.sleep(0.1)
            if L < R:
                self.PWM.setMotorModel(1000, 1000, -1000, -1000)
            else:
                self.PWM.setMotorModel(-1000, -1000, 1000, 1000)
        elif L < 30 and M < 30:
            PWM.setMotorModel(1500, 1500, -1500, -1500)
        elif R < 30 and M < 30:
            PWM.setMotorModel(-1500, -1500, 1500, 1500)
        elif L < 20:
            PWM.setMotorModel(1500, 1500, -500, -500)
            if L < 10:
                PWM.setMotorModel(1500, 1500, -1000, -1000)
        elif R < 20:
            PWM.setMotorModel(-500, -500, 1500, 1500)
            if R < 10:
                PWM.setMotorModel(-1000, -1000, 1000, 1000)
        else:
            self.PWM.setMotorModel(600, 600, 600, 600)
    """
    def run_motor(self, L, M, R):
        if (L < 30 and M < 30 and R < 30) :
            self.PWM.setMotorModel(-1000, -1000, -1000, -1000)
            time.sleep(0.1)
            if L < R:
                self.PWM.setMotorModel(2000, 2000, -2000, -2000)
            else:
                self.PWM.setMotorModel(-2000, -2000, 2000, 2000)
        elif L < 30 and M < 30:
            PWM.setMotorModel(2500, 2500, -2500, -2500)
        elif R < 30 and M < 30:
            PWM.setMotorModel(-2500, -2500, 2500, 2500)
        elif L < 20:
            PWM.setMotorModel(2500, 2500, -1500, -1500)
            if L < 10:
                PWM.setMotorModel(2500, 2500, -2000, -2000)
        elif R < 20:
            PWM.setMotorModel(-1500, -1500, 2500, 2500)
            if R < 10:
                PWM.setMotorModel(-2000, -2000, 2000, 2000)
        else:
            self.PWM.setMotorModel(600, 600, 600, 600)
    
    def run(self):
        self.PWM = Motor()
        self.pwm_S = Servo()
        while True:
            M = self.get_distance()
            if M <= 15:
                self.PWM.setMotorModel(-1500, -1500, -1500, -1500)
                time.sleep(0.2)  
                if 50 >= random.randint(1, 100):
                    self.PWM.setMotorModel(-2000, -2000, 2000, 2000)
                else:
                    self.PWM.setMotorModel(2000, 2000, -2000, -2000)
                time.sleep(0.5)  

            elif 15 < M <= 25:
                self.PWM.setMotorModel(0, 0, 0, 0)
                time.sleep(0.2)
                if 50 >= random.randint(1, 100):
                    self.PWM.setMotorModel(-2000, -2000, 2000, 2000)
                else:
                    self.PWM.setMotorModel(2000, 2000, -2000, -2000)
                time.sleep(0.5)
            else:  
                self.PWM.setMotorModel(1000, 1000, 1000, 1000)
    
    """
    def run(self):
        self.PWM = Motor()
        self.pwm_S = Servo()

        while True:
            #self.pwm_S.setServoPwm("0", 90)
            #time.sleep(0.1)
            M = self.get_distance()
            time.sleep(0.2)
            if 0< M < 30:
                PWM.setMotorModel(0, 0, 0, 0)
                time.sleep(0.1)
                self.pwm_S.setServoPwm("0", 30)
                time.sleep(0.5)
                L = self.get_distance()
                time.sleep(0.5)
                self.pwm_S.setServoPwm("0", 151)
                time.sleep(0.5)
                R = self.get_distance()
                time.sleep(0.5)
                if L <= R:
                    self.PWM.setMotorModel(2000, 2000, -2000, -2000)
                    self.pwm_S.setServoPwm("0", 90)
                    time.sleep(0.3)
                else:
                    self.PWM.setMotorModel(-2000, -2000, 2000, 2000)
                    self.pwm_S.setServoPwm("0", 90)
                    time.sleep(0.3)
            else:
                self.PWM.setMotorModel(600, 600, 600, 600) 
    """
    
    
    """
    def run(self):
        self.PWM = Motor()
        self.pwm_S = Servo()

        while True:
            self.pwm_S.setServoPwm("0", 90)
            #time.sleep(0.1)
            M = self.get_distance()
            time.sleep(0.5)
            if M < 30:
                PWM.setMotorModel(0, 0, 0, 0)
                self.pwm_S.setServoPwm("0", 30)
                L = self.get_distance()
                time.sleep(1)
                self.pwm_S.setServoPwm("0", 151)
                R = self.get_distance()
                time.sleep(1)
                self.run_motor(L, M, R)
                self.pwm_S.setServoPwm("0", 90)
            else:
                self.run_motor(20, M, 20)
    """
    """
    def run(self):
        self.PWM = Motor()
        self.pwm_S = Servo()
        while True:
            OBSTACLE_THRESHOLD = 50  # 假设小于0.5米的障碍物需要避障
            self.pwm_S.setServoPwm("0", 90)
            time.sleep(0.1)
            M = self.get_distance()
            time.sleep(0.5)
            if M < OBSTACLE_THRESHOLD:
                # 如果正前方有障碍物
                self.PWM.setMotorModel(0, 0, 0, 0)
                time.sleep(0.1)
                self.pwm_S.setServoPwm("0", 30)
                L = self.get_distance()
                time.sleep(1)
                self.pwm_S.setServoPwm("0", 151)
                R = self.get_distance()
                time.sleep(1)
                self.pwm_S.setServoPwm("0", 90)
                if M < OBSTACLE_THRESHOLD:
                    # 如果左前方和右前方都有障碍物，选择距离较远的方向转向
                    if L < OBSTACLE_THRESHOLD and R < OBSTACLE_THRESHOLD:
                        if L > R:
                            self.PWM.setMotorModel(-2000, -2000, 2000, 2000)  # 左转，因为左方距离更远
                        else:
                            self.PWM.setMotorModel(2000, 2000, -2000, -2000)  # 右转，因为右方距离更远
                    # 如果只有一侧有障碍物或一侧距离更远，则向另一侧转向
                    elif L < OBSTACLE_THRESHOLD:
                        self.PWM.setMotorModel(2000, 2000, -2000, -2000)  # 右方没有障碍物或距离更远
                    else:
                        self.PWM.setMotorModel(-2000, -2000, 2000, 2000)  # 左方没有障碍物或距离更远
                # 如果正前方没有障碍物，但两侧有障碍物
                elif L < OBSTACLE_THRESHOLD or R < OBSTACLE_THRESHOLD:
                # 向没有障碍物或距离更远的一侧转向
                    if L > R:
                        self.PWM.setMotorModel(-2000, -2000, 2000, 2000)  # 左转，因为左方距离更远
                    else:
                        self.PWM.setMotorModel(2000, 2000, -2000, -2000)  # 右转，因为右方距离更远
            # 如果所有方向都没有障碍物，继续前进
            else:
                self.PWM.setMotorModel(600, 600, 600, 600)
    """
    def run0(self):
        self.PWM = Motor()
        self.pwm_S = Servo()

        for i in range(30, 151, 60):
            self.pwm_S.setServoPwm('0', i)
            time.sleep(0.2)
            if i == 30:
                L = self.get_distance()
            elif i == 90:
                M = self.get_distance()
            else:
                R = self.get_distance()
        while True:
            for i in range(90, 30, -60):
                self.pwm_S.setServoPwm('0', i)
                time.sleep(0.2)
                if i == 30:
                    L = self.get_distance()
                elif i == 90:
                    M = self.get_distance()
                else:
                    R = self.get_distance()
                self.run_motor(L, M, R)
            for i in range(30, 151, 60):
                self.pwm_S.setServoPwm('0', i)
                time.sleep(0.2)
                if i == 30:
                    L = self.get_distance()
                elif i == 90:
                    M = self.get_distance()
                else:
                    R = self.get_distance()
                self.run_motor(L, M, R)


ultrasonic = Ultrasonic()
# Main program logic follows:
if __name__ == '__main__':
    print('Program is starting ... ')
    try:
        ultrasonic.run()
    except KeyboardInterrupt:  # When 'Ctrl+C' is pressed, the child program destroy() will be  executed.
        PWM.setMotorModel(0, 0, 0, 0)
        ultrasonic.pwm_S.setServoPwm('0', 90)
